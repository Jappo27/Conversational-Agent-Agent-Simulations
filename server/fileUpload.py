...
def convert_pdf_to_text(file_path, file_destination):
    if file_path:
        with open(file_path, 'rb') as pdf_file:
            ...
            if current_chunk:  # Don't forget the last chunk!
                chunks.append(current_chunk)
            with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),file_destination), "a", encoding="utf-8") as vault_file:
                for chunk in chunks:
...
def upload_txtfile(file_path, file_destination):
    if file_path:
...
def upload_jsonfile(file_path, file_destination):
    if file_path:
