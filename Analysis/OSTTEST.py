...
def plotTTestResults(tStats, pValues, dfList, alpha=0.05):
    ...
    tCoef = np.polyfit(turns, tStats, 1)
    tTrend = np.polyval(tCoef, turns)

    pCoef = np.polyfit(turns, pValues, 1)
    pTrend = np.polyval(pCoef, turns)

    fig, ax = plt.subplots(2, 1, figsize=(12, 8), sharex=True)

    ax[0].plot(turns, tStats, marker='o', color="blue", label="t-statistic")
    ax[0].plot(turns, tTrend, color="green", linewidth=2, label="t-stat trend")
    ...
    ax[1].plot(turns, pTrend, color="green", linewidth=2, label="p-value trend")
...
print(pragmaticTList, pragmaticPList, pragmaticDfList)
